...
def plot_training_progress(goal_stats_file: str, loss_history: Optional[List[float]] = None, 
    ...
    # Calculate cumulative moves (total moves up to each goal)
    cumulative_moves = np.cumsum([0] + moves_per_goal[:-1])
    ...
    ax1.plot(goal_numbers, moves_per_goal, 'o-', color='#2563EB', linewidth=2, markersize=6, alpha=0.8)
    # Add moving average
    if len(moves_per_goal) > 5:
        ...
        ax1.plot(goal_numbers[window-1:], moving_avg, '--', color='#3B82F6', linewidth=2, alpha=0.6, label=f'{window}-goal MA')
        ax1.legend()
    ax1.set_xlabel('Goal Number')
    ...
    ax2.plot(goal_numbers, agency_means, 'o-', color='#F59E0B', linewidth=2, markersize=6, alpha=0.8, label='Mean')
    ax2.fill_between(goal_numbers, 
                     np.array(agency_means) - np.array(agency_stds),
                     np.array(agency_means) + np.array(agency_stds),
                     alpha=0.2, color='#F59E0B', label='±1 std')
    ax2.set_xlabel('Goal Number')
    ...
    ax3.plot(goal_numbers, segment_length_means, 'o-', color='#10B981', linewidth=2, markersize=6, alpha=0.8, label='Mean')
    ax3.fill_between(goal_numbers,
                     np.array(segment_length_means) - np.array(segment_length_stds),
                     np.array(segment_length_means) + np.array(segment_length_stds),
                     alpha=0.2, color='#10B981', label='±1 std')
    ax3.set_xlabel('Goal Number')
    ...
    ax5.plot(goal_numbers, agency_stds, 'o-', color='#EF4444', linewidth=2, markersize=6, alpha=0.8)
    ax5.set_xlabel('Goal Number')
