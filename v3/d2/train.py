...
if __name__ == "__main__":
    ...
    ACTOR_CONFIG = {
        ...
        "token_embed_dim": TOKEN_EMBED_DIM,
        "n_attention_heads": 4,
        "stepwise_knot_generation": True  # Set to True to generate one knot at a time with stop decisions
    }
    ...
    goals = generate_goals_2d(num_goals=3, bounds=bounds, seed=42)
    ...
    for pos, radius in obstacles:
        if len(pos) == 2:
            obstacles_config.append([pos[0], pos[1], 0.0, radius])  # [x, y, z=0, radius]
        else:
            obstacles_config.append([pos[0], pos[1], pos[2] if len(pos) > 2 else 0.0, radius])
