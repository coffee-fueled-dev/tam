...
class TknHead:
    ...
    def __init__(
        self, name, quantization_bins=11, quant_range=(-2.0, 2.0), vocab_size=65536
    ):
        self.name = name
    ...
    def get_head_state(self):
        ...
        if buffer_str:
            buffer_hash = (
                int(hashlib.md5(buffer_str.encode()).hexdigest(), 16) % self.vocab_size
            )
        else:
            buffer_hash = 0
...
class MarkovLattice:
    ...
    def tokenize_with_markov(
        self, head_name, pattern, hub_count, surprise, previous_token=None
    ):
        """
        ...
        pattern_hash = (
            int(hashlib.md5(pattern_str.encode()).hexdigest(), 16) % self.vocab_size
        )
        ...
        if previous_token is None:
            ...
            if transition_key in self.markov_transitions:
                ...
                transition_hash = (
                    int(hashlib.md5(str(transition_key).encode()).hexdigest(), 16)
                    % self.vocab_size
                )
    ...
    def _update_importance_score(self, hub_id):
        ...
        max_in_degree = max(
            (h["in_degree"] for h in self.hub_graph.values()), default=1
        )
        max_hub_count = max(
            (h["hub_count"] for h in self.hub_graph.values()), default=1
        )
        max_transition_count = max(
            (h["transition_count"] for h in self.hub_graph.values()), default=1
        )
        ...
        norm_transition_freq = (
            hub["transition_count"] / max_transition_count
            ...
            if emission_data is not None:
                ...
                if pattern is not None:
                    ...
                    if current_pos is not None and token_id in self.hub_metadata:
                        current_pos_copy = (
                            current_pos.copy()
                            if hasattr(current_pos, "copy")
                            else np.array(current_pos)
                        )
                        self.hub_metadata[token_id]["position_history"].append(
                            current_pos_copy
                        )
    ...
    def query_trajectory(self, trajectory_points, current_pos):
        ...
        if isinstance(current_pos, torch.Tensor):
            current_pos_np = (
                ...
                if current_pos.requires_grad
                else current_pos.cpu().numpy()
            )
        else:
        ...
        for traj_point in traj_np:
            ...
            for hub_id, metadata in self.hub_metadata.items():
                if (
                    "position_history" in metadata
                    and len(metadata["position_history"]) > 0
                ):
...
class UnifiedTknProcessor:
    ...
    def __init__(
        ...
        self.unified_head = TknHead(
            "unified", quantization_bins, quant_range, vocab_size
        )

        # Proximity head (still separate, as it's not dimension-specific)
        self.proximity_head = TknHead(
            "proximity",
            quantization_bins,
            quant_range=(-5.0, 10.0),
    ...
    def _ensure_initialized(self, state_dim):
        ...
        self.head_names = [f"dim_{i}" for i in range(state_dim)] + [
            "proximity",
            "energy",
        ]
        self.lattice = MarkovLattice(
            self.head_names,
    ...
    def process_observation(
        ...
        raw_obs,
        obstacles,
        max_observed_obstacles=None,
        max_observed_goals=None,
    ):
        """
        ...
        Args:
            ...
            max_observed_obstacles: Maximum number of obstacles in observation (None = infer from raw_obs)
            max_observed_goals: Maximum number of goals in observation (None = infer from raw_obs)
        ...
        obs_start_idx = 0
        obstacle_size = (
            inferred_state_dim + 2
        )  # rel_pos (state_dim) + radius (1) + color (1)

        # If max_observed_obstacles is None, infer from raw_obs by scanning until we find no more obstacles
        inferred_max_obstacles = max_observed_obstacles
        if max_observed_obstacles is None:
            # Scan until we find an invalid obstacle (zero radius or wrong color)
            i = 0
            while True:
                ...
                # Check if obstacle exists (radius > small threshold and color = 1.0 for obstacles)
                if obs_radius > 1e-6 and abs(obs_color - 1.0) < 1e-6:
                    obstacle_directions.append(rel_obs_pos)
                    i += 1
                else:
                    # Found padding/empty slot, stop scanning
                    break
            inferred_max_obstacles = i
        else:
            # Use specified limit
        ...
        goal_start_idx = inferred_max_obstacles * obstacle_size
        goal_size = (
            inferred_state_dim + 2
        )  # rel_pos (state_dim) + radius (1) + color (1)

        # If max_observed_goals is None, infer from raw_obs by scanning until we find no more goals
        inferred_max_goals = max_observed_goals
        if max_observed_goals is None:
            # Scan until we find an invalid goal (zero radius or wrong color)
            i = 0
            while True:
                ...
                # Check if goal exists (radius > small threshold and color = -1.0 for goals)
        ...
        boundary_start_idx = (
            inferred_max_obstacles * obstacle_size + inferred_max_goals * goal_size
        )
        ...
        if (
            energy_start_idx < len(raw_obs_np)
            and len(raw_obs_np) >= energy_start_idx + 2
        ):
        ...
        for dim_idx in range(inferred_state_dim):
            ...
            goal_q = self.unified_head.quantize(
                rel_goal_np[dim_idx] if dim_idx < len(rel_goal_np) else 0.0
            )
            ...
            if nearest_obs_dir is not None:
                obs_dir_component = (
                    nearest_obs_dir[dim_idx] if dim_idx < len(nearest_obs_dir) else 0.0
                )
            else:
            ...
            if boundary_distances is not None and dim_idx < boundary_distances.shape[0]:
                ...
                boundary_pattern, boundary_novel = self.proximity_head.process(
                    boundary_q
                )
            else:
            ...
            head_emissions[head_name] = (
                combined_pattern if combined_pattern else [delta_q],
                combined_novel,
            )
            hub_counts_dict[head_name] = hub_count
            surprises_dict[head_name] = surprise

            dimension_traits.append(
                torch.tensor([[hub_count, surprise]], dtype=torch.float32)
            )  # (1, 2)
            ...
            if obs_dir_pattern is not None:
                head_emissions[f"{head_name}_obs_dir"] = (
                    obs_dir_pattern,
                    obs_dir_novel,
                )
            if boundary_pattern is not None:
                head_emissions[f"{head_name}_boundary"] = (
                    boundary_pattern,
                    boundary_novel,
                )
        ...
        if obstacles:
            for obs_p, obs_r in obstacles:
                ...
                if len(obs_pos) < inferred_state_dim:
                    obs_pos = np.pad(
                        obs_pos, (0, inferred_state_dim - len(obs_pos)), mode="constant"
                    )
        ...
        proximity_buffer_hash, proximity_hub_count, proximity_surprise = (
            self.proximity_head.get_head_state()
        )

        # Store proximity for lattice.update()
        head_name = "proximity"
        primary_pattern = (
            proximity_pattern if proximity_pattern is not None else [proximity_q]
        )
        ...
        proximity_traits = torch.tensor(
            [[proximity_hub_count, proximity_surprise]], dtype=torch.float32
        )
        ...
        if energy_value is not None and energy_normalized is not None:
            ...
            energy_value_pattern, energy_value_novel = self.proximity_head.process(
                energy_value_q
            )

            # Quantize energy_normalized using unified_head (0-1 range)
            energy_normalized_q = self.unified_head.quantize(energy_normalized)
            energy_normalized_pattern, energy_normalized_novel = (
                self.unified_head.process(energy_normalized_q)
            )

            # Use normalized energy as primary pattern (more informative for learning)
            energy_pattern = (
                energy_normalized_pattern
                if energy_normalized_pattern is not None
                else [energy_normalized_q]
            )
            energy_novel = energy_normalized_novel

            # Get head state for energy (use unified_head since we're using normalized)
            energy_buffer_hash, energy_hub_count, energy_surprise = (
                self.unified_head.get_head_state()
            )
            ...
            energy_traits = torch.tensor(
                [[energy_hub_count, energy_surprise]], dtype=torch.float32
            )
            ...
            if name != "proximity" and name != "energy":
                ...
                if emission_data is not None and self.lattice is not None:
                    ...
                    if pattern is not None and len(pattern) > 0:
                        ...
                        previous_token = (
                            self.lattice.current_lattice[i].item()
                            if hasattr(self.lattice, "current_lattice")
                            else None
                        )
                        ...
                        dimension_tokens.append(
                            torch.tensor(dim_tokens, dtype=torch.long)
                        )  # Variable length: (N,)
                        ...
                        hub_importance = (
                            self.lattice.get_hub_importance(token_id)
                            if self.lattice
                            else 0.0
                        )
                        is_hub = (
                            token_id in self.lattice.hub_graph
                            if self.lattice
                            else False
                        )
                        hub_count_prop = 0.0
                    ...
                    is_hub = (
                        token_id in self.lattice.hub_graph if self.lattice else False
                    )
                    hub_count_prop = 0.0
                    in_degree = 0.0
                    if is_hub and self.lattice:
        ...
        return {
            ...
            "lattice_traits": torch.cat(
                dimension_traits + [proximity_traits, energy_traits], dim=0
            ),  # (num_heads, 2)
            "buffer_hashes": torch.cat(
                [
                    torch.tensor(
                        [self.unified_head.get_head_state()[0]] * inferred_state_dim
                        + [self.proximity_head.get_head_state()[0]]
                        + [self.unified_head.get_head_state()[0]],
                        dtype=torch.long,
                    )
                ],
                dim=0,
            ...
            "metadata": {
                ...
                "nearest_obstacle_dir": nearest_obs_dir.tolist()
                if nearest_obs_dir is not None
                else None,
                "num_obstacles_detected": len(obstacle_directions),
                "boundary_distances": boundary_distances.tolist()
                if boundary_distances is not None
                else None,
                "quantized": quantized_values,
                "emissions": {
                    k: (v[0] if v is not None else None)
                    for k, v in head_emissions.items()
                },
                "has_emissions": any(v is not None for v in head_emissions.values()),
                "has_surprise": surprise_mask.any().item()
                if isinstance(surprise_mask, torch.Tensor)
                else any(surprise_mask),
            },
        }
    ...
    def reset_episode(self):
        ...
        self.unified_head = TknHead(
            "unified", self.quantization_bins, self.quant_range, self.vocab_size
        )
        self.proximity_head = TknHead(
            "proximity",
