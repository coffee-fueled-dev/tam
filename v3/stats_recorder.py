...
class StatsRecorder(ABC):
    ...
    def record_move(
        ...
        mu_t: Optional[
            Any
        ] = None,  # Selected next step (1, state_dim) - single step for single-step generation
        sigma_t: Optional[
            Any
        ] = None,  # Selected sigma (1, state_dim) - per-dimension precision [sigma_x, sigma_y, ...]
        goals_reached: Optional[
            int
        ] = None,  # Number of goals reached in this move (0 or 1 typically)
        situation_complexity: Optional[
            float
        ] = None,  # Optional situation complexity for tracking
        total_loss: Optional[float] = None,  # Total loss value
        goal_reward: Optional[float] = None,  # Goal reward value
        goal_penalty: Optional[float] = None,  # Goal penalty value
        loss_weights: Optional[Dict[str, float]] = None,  # Loss weights for computing weighted components
    ):
        """
        ...
        Args:
            ...
            agency_reward: Agency reward value (positive reward for agency)
            knot_components: Optional dict with step-related metrics:
                - selected_port: Index of selected port
                - step_length: Length of the step
                - obstacle_proximity: Distance to nearest obstacle
                - obstacle_collision: Whether collision occurred
            mu_t: Selected next step (1, state_dim) tensor or array - single step for single-step generation
            sigma_t: Selected sigma (1, state_dim) tensor or array - per-dimension precision [sigma_x, sigma_y, ...]
                     For 2D: [[sigma_x, sigma_y]] - anisotropic affordance cone
            goals_reached: Optional number of goals reached in this move (for cumulative tracking)
            situation_complexity: Optional situation complexity score (for tracking)
            total_loss: Optional total loss value
            goal_reward: Optional goal reward value
            goal_penalty: Optional goal penalty value
            loss_weights: Optional loss weights dict for computing weighted components
        """
        pass
...
class JSONLStatsRecorder(StatsRecorder):
    ...
    def record_move(
        ...
        if hasattr(agency_reward, "item"):
            agency_reward = agency_reward.item()
        ...
        if sigma_t is not None:
            if torch is not None and isinstance(sigma_t, torch.Tensor):
                ...
                sigma_t_list = (
                    list(sigma_t) if isinstance(sigma_t, (list, tuple)) else None
                )
        ...
        entry = {
            ...
            "agency_reward": float(agency_reward),
        }

        if situation_complexity is not None:
            if hasattr(situation_complexity, "item"):
                situation_complexity = situation_complexity.item()
            entry["situation_complexity"] = float(situation_complexity)
        ...
        if total_loss is not None:
            ...
            entry["total_loss"] = float(total_loss)

        if goal_reward is not None:
            if hasattr(goal_reward, "item"):
                goal_reward = goal_reward.item()
            entry["goal_reward"] = float(goal_reward)

        if goal_penalty is not None:
            if hasattr(goal_penalty, "item"):
                goal_penalty = goal_penalty.item()
            entry["goal_penalty"] = float(goal_penalty)
    ...
    def generate_plots(self):
        ...
        Creates plots for:
        - Total loss and components over moves
        ...
        # Handle both old format (agency_cost) and new format (agency_reward)
        agency_rewards = [
            entry.get("agency_reward", entry.get("agency_cost", 0.0))
        ...
        num_plots = 1  # Combined total loss plot (replaces separate binding_loss and agency_reward plots)
        if has_knot_components:
        ...
        if has_obstacle_data:
            num_plots += (
                2  # obstacle proximity vs binding loss, conditional collision rate
            )

        # Calculate number of other plots (excluding loss plot)
        num_other_plots = num_plots - 1
        
        # Create custom layout: loss plot full width on top, other plots side by side below
        if num_other_plots == 0:
            # Only loss plot
            fig, ax = plt.subplots(1, 1, figsize=(14, 6))
            fig.suptitle("Training Statistics", fontsize=16, fontweight="bold")
            axes_list = [ax]
        else:
            # Custom layout with gridspec: loss plot full width, other plots in row below
            # Use number of columns equal to number of other plots
            num_cols = num_other_plots
            fig = plt.figure(figsize=(6 * max(num_cols, 2), 10))
            fig.suptitle("Training Statistics", fontsize=16, fontweight="bold")
            gs = gridspec.GridSpec(2, num_cols, figure=fig, hspace=0.3, wspace=0.3, 
                                   height_ratios=[1.5, 1])  # Loss plot taller
            
            # Loss plot spans all columns in first row
            ax_loss_full = fig.add_subplot(gs[0, :])
            axes_list = [ax_loss_full]
            
            # Other plots in second row, side by side
        ...
        # Plot 1: Combined Total Loss with all components
        ax_loss = get_next_axis()
        
        # Extract loss data
        total_losses = [entry.get("total_loss") for entry in stats_data]
        goal_rewards = [entry.get("goal_reward", 0.0) for entry in stats_data]
        goal_penalties = [entry.get("goal_penalty", 0.0) for entry in stats_data]
        
        # Get loss weights (from first entry or defaults)
        loss_weights = stats_data[0].get("loss_weights", {
            "binding_loss": 1.0,
            "agency_reward": 1.0,
            "goal_reward": 1.0,
            "goal_penalty": 1.0,
        ...
        binding_losses_weighted = [
            loss_weights.get("binding_loss", 1.0) * bl 
            for bl in binding_losses
        ]
        agency_rewards_weighted = [
            -loss_weights.get("agency_reward", 1.0) * ar 
            for ar in agency_rewards
        ]
        goal_rewards_weighted = [
            -loss_weights.get("goal_reward", 1.0) * gr 
            for gr in goal_rewards
        ]
        goal_penalties_weighted = [
            loss_weights.get("goal_penalty", 1.0) * gp 
            for gp in goal_penalties
        ]
        
        # If total_loss is missing, compute from components
        if any(tl is None for tl in total_losses):
            total_losses = [
                bl_w + ar_w + gr_w + gp_w
                for bl_w, ar_w, gr_w, gp_w in zip(
                    binding_losses_weighted,
                    agency_rewards_weighted,
                    goal_rewards_weighted,
                    goal_penalties_weighted
        ...
        valid_indices = [
            i for i, tl in enumerate(total_losses) 
            if tl is not None
        ]
        if valid_indices:
            valid_moves = [moves[i] for i in valid_indices]
            valid_total_losses = [total_losses[i] for i in valid_indices]
            valid_binding_losses_weighted = [binding_losses_weighted[i] for i in valid_indices]
            valid_agency_rewards_weighted = [agency_rewards_weighted[i] for i in valid_indices]
            valid_goal_rewards_weighted = [goal_rewards_weighted[i] for i in valid_indices]
            valid_goal_penalties_weighted = [goal_penalties_weighted[i] for i in valid_indices]
            
            # Use stacked area chart to clearly separate components
            # Stack order: binding_loss (base), then agency_reward, goal_reward, goal_penalty on top
            # Note: agency_reward and goal_reward are negative, so they reduce the stack
            
            # Start with binding loss as base
            bottom_binding = [0.0] * len(valid_moves)
            top_binding = valid_binding_losses_weighted
            
            # Agency reward (negative) - subtracts from binding loss
            bottom_agency = top_binding
            top_agency = [
                b + a for b, a in zip(top_binding, valid_agency_rewards_weighted)
            ]
            
            # Goal reward (negative) - subtracts further
            bottom_goal_reward = top_agency
            top_goal_reward = [
                b + g for b, g in zip(top_agency, valid_goal_rewards_weighted)
            ]
            
            # Goal penalty (positive) - adds on top
            bottom_penalty = top_goal_reward
            top_penalty = [
                b + p for b, p in zip(top_goal_reward, valid_goal_penalties_weighted)
            ]
            ...
            # Plot total loss line on top (thick, prominent)
            ax_loss.plot(
                valid_moves,
                valid_total_losses,
                "k-",
            ...
            if len(valid_total_losses) >= window_size:
                moving_avg = np.convolve(
                    valid_total_losses,
                    np.ones(window_size) / window_size,
                    mode="valid",
                )
                moving_avg_moves = valid_moves[window_size - 1 :]
                ax_loss.plot(
                    moving_avg_moves,
                    moving_avg,
                    "k--",
                    linewidth=2,
                    alpha=0.8,
                    label=f"Total Loss MA ({window_size})",
                    zorder=11,  # On top of total loss line
                )
        
        ax_loss.set_xlabel("Move")
        ax_loss.set_ylabel("Loss")
        ax_loss.set_title("Total Loss and Components Over Moves")
        ...
        if has_knot_components:
                    ...
                    for key, value in entry["knot_components"].items():
                        ...
                        if isinstance(value, (int, float)):
                            ...
                            if len(value) > 0 and isinstance(value[0], (int, float)):
                                knot_data[key].append(
                                    np.mean(value) if len(value) > 1 else value[0]
                                )
                            else:
            ...
            if "selected_port" in knot_data and len(knot_data["selected_port"]) > 0:
                ...
                valid_indices = [
                    i
                    for i, v in enumerate(knot_data["selected_port"])
                    if not np.isnan(v)
                ]
                if valid_indices:
                    valid_moves = [moves[i] for i in valid_indices]
                    ...
                    if unique_ports:
                        ax3.set_yticks(
                            unique_ports
                        )  # Set y-ticks to actual port values
                    ax3.grid(True, alpha=0.3, axis="y")

            # Plot num_active_knots (if available) - use scatter plot for discrete values
            if (
                "num_active_knots" in knot_data
                and len(knot_data["num_active_knots"]) > 0
            ):
                ax4 = get_next_axis()
                # Filter out NaN values for plotting
                valid_indices = [
                    i
                    for i, v in enumerate(knot_data["num_active_knots"])
                    if not np.isnan(v)
                ]
                if valid_indices:
                    valid_moves = [moves[i] for i in valid_indices]
                    valid_knots = [
                        knot_data["num_active_knots"][i] for i in valid_indices
                    ]
                    ax4.scatter(
                        valid_moves,
                        valid_knots,
                    ...
                    if unique_knots:
                        ax4.set_yticks(
                            unique_knots
                        )  # Set y-ticks to actual knot values
            ...
            if has_segment_length_data:
                ...
                for entry in stats_data:
                    if (
                        "knot_components" in entry
                        and "segment_length_mean" in entry["knot_components"]
                    ):
                        seg_length_min.append(
                            entry["knot_components"].get("segment_length_min", np.nan)
                        )
                        seg_length_max.append(
                            entry["knot_components"].get("segment_length_max", np.nan)
                        )
                        seg_length_mean.append(
                            entry["knot_components"].get("segment_length_mean", np.nan)
                        )
                        seg_length_std.append(
                            entry["knot_components"].get("segment_length_std", np.nan)
                        )
                    else:
                ...
                # Filter out NaN values for plotting
                valid_indices = [
                    i for i, v in enumerate(seg_length_mean) if not np.isnan(v)
                ]
                if valid_indices:
                    valid_moves = [moves[i] for i in valid_indices]
        ...
        if has_obstacle_data:
            ...
            valid_indices = [
                i
                for i, p in enumerate(obstacle_proximities)
                if not np.isnan(p) and p < 100.0
            ]
            if valid_indices:
                valid_moves = [moves[i] for i in valid_indices]
                ...
                if len(valid_proximities) >= window_size:
                    ...
                    moving_avg_loss = np.convolve(
                        sorted_loss, np.ones(window_size) / window_size, mode="valid"
                    )
            ...
            for bin_name in bin_names:
                ...
                if len(bin_indices) >= window_size:
                    ...
                    moving_collision_rate = np.convolve(
                        bin_collisions, np.ones(window_size) / window_size, mode="valid"
                    )
                    moving_moves = bin_moves[window_size - 1 :]
                    collision_rates_by_bin[bin_name] = (
                        moving_moves,
                        moving_collision_rate,
                    )
            ...
            for bin_name in bin_names:
                if collision_rates_by_bin[bin_name]:
                    ...
                    ax_obs2.plot(
                        moves_bin,
                        rates,
                        color=colors_bins.get(bin_name, "gray"),
                        linewidth=1.5,
                        alpha=0.7,
                        label=f"{bin_name}",
                    )
        ...
        # Hide any remaining unused subplots (shouldn't happen with new layout, but keep for safety)
        while plot_idx < len(axes_list):
            axes_list[plot_idx].axis("off")
            plot_idx += 1
