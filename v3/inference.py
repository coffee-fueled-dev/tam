...
class TransformerInferenceEngine(nn.Module):
    ...
    def __init__(
        ...
        self.output_proj = nn.Sequential(
            nn.Linear(token_embed_dim, latent_dim), nn.LayerNorm(latent_dim)
        )
    ...
    def forward(
        ...
        for dim_idx in range(state_dim):
            ...
            if isinstance(dimension_tokens, list):
                ...
                if dimension_structural_props is not None and dim_idx < len(
                    dimension_structural_props
                ):
                ...
                for token_idx in range(num_tokens):
                    ...
                    if dim_struct_props is not None and token_idx < len(
                        dim_struct_props
                    ):
                        ...
                        struct_prop_vec = struct_prop_vec.unsqueeze(0).expand(
                            B, -1
                        )  # (B, 4)
                        struct_embed = self.struct_prop_proj(
                            struct_prop_vec
                        )  # (B, token_embed_dim)
                        token_embed = (
                            token_embed + struct_embed
                        )  # Combine structural properties with token embedding

                    dim_token_embeds.append(token_embed)

                # Aggregate multiple tokens per dimension (mean pooling)
                if len(dim_token_embeds) > 1:
                    token_embed = torch.stack(dim_token_embeds, dim=0).mean(
                        dim=0
                    )  # (B, token_embed_dim)
                else:
                ...
                dim_token_ids = (
                    dimension_tokens[:, dim_idx]
                    if dimension_tokens.dim() > 1
                    else dimension_tokens
                )
                if dim_token_ids.dim() == 1 and dim_token_ids.shape[0] > 1:
                    ...
                    for token_idx in range(num_tokens):
                        token_id = (
                            dim_token_ids[token_idx].unsqueeze(0).expand(B)
                        )  # (B,)
                        token_embed_single = self.token_embedding(
                            token_id
                        )  # (B, token_embed_dim)
                        token_embeds_list.append(token_embed_single)
                    if len(token_embeds_list) > 1:
                        token_embed = torch.stack(token_embeds_list, dim=0).mean(
                            dim=0
                        )  # (B, token_embed_dim)
                    else:
            ...
            dim_pos = self.dim_embedding(
                torch.tensor(dim_idx, device=device)
            )  # (token_embed_dim,)
            ...
            if isinstance(dimension_traits, list):
                ...
                dim_trait_tensor = dimension_traits[
                    dim_idx
                ]  # (1, 2) from UnifiedTknProcessor
                ...
                dim_traits = (
                    dimension_traits[:, dim_idx, :]
                    if dimension_traits.dim() == 3
                    else dimension_traits
                )
            ...
            dim_token = (
                token_embed + dim_pos + trait_embed + intent_embed
            )  # Should be (B, token_embed_dim)
            ...
            if dim_token.dim() == 1:
                ...
                if B == 1:
                    ...
                    dim_token = dim_token.unsqueeze(0).expand(
                        B, -1
                    )  # (B, token_embed_dim)
        ...
        sequence_with_cls = torch.cat(
            [cls, sequence], dim=1
        )  # (B, state_dim+1, token_embed_dim)
        ...
        if memory_context is not None:
            ...
            sequence_with_memory = torch.cat(
                [sequence_with_cls, memory_context], dim=1
            )  # (B, state_dim+1+memory_window, token_embed_dim)
        ...
        if memory_context is not None:
            ...
            current_situation_embed = situation_sequence.mean(
                dim=1, keepdim=True
            )  # (B, 1, token_embed_dim)
            new_memory_context = torch.cat(
                [memory_context, current_situation_embed], dim=1
            )  # (B, memory_window+1, token_embed_dim)
            # Keep only last memory_window entries
            new_memory_context = new_memory_context[
                :, -self.memory_window :, :
            ]  # (B, memory_window, token_embed_dim)
        else:
            ...
            if self.memory_window > 1:
                ...
                new_memory_context = current_situation_embed.repeat(
                    1, self.memory_window, 1
                )  # (B, memory_window, token_embed_dim)
            else:
