...
- **`environment.py`**: `Simple3DEnvironment` - A clean 3D point-mass environment implementation
- **`train.py`**: Training script that uses `train_tam_system()` with the new environment
- **`__init__.py`**: Module exports

## Features

The `Simple3DEnvironment` implements the abstract `Environment` interface:
...
# Create environment
env = Simple3DEnvironment(
    state_dim=3,
    bounds={'min': [-5.0, -5.0, -5.0], 'max': [5.0, 5.0, 5.0]},
    obstacles=[([1.0, 1.0, 1.0], 0.5), ([2.0, -1.0, 0.5], 0.3)],
    goals=[torch.tensor([3.0, 3.0, 3.0]), torch.tensor([-2.0, 4.0, -1.0])]
)
