"""
Training script for Simple3DEnvironment using the generic train_tam_system().
...
from v3.d3.environment import Simple3DEnvironment
import os
...
def generate_obstacles_3d(num_obstacles: int = 5, bounds: dict = None, seed: int = 42):
    """Generate simple 3D obstacles."""
    if bounds is None:
...
def generate_goals_3d(num_goals: int = 3, bounds: dict = None, seed: int = 42):
    """Generate simple 3D goals."""
    if bounds is None:
        bounds = {'min': [-5.0, -5.0, -5.0], 'max': [5.0, 5.0, 5.0]}
...
if __name__ == "__main__":
    print("=" * 80)
    print("TAM v3 - Simple 3D Environment Training")
    print("=" * 80)
    ...
    obstacles = generate_obstacles_3d(num_obstacles=20, bounds=bounds, seed=60)
    goals = generate_goals_3d(num_goals=3, bounds=bounds, seed=42)
    ...
    # Create environment
    environment = Simple3DEnvironment(
        state_dim=STATE_DIM,
        bounds=bounds,
        obstacles=obstacles,
        goals=goals,
    ...
    run_dir = os.path.join(artifacts_dir, f"d3_run_{session_timestamp}")
    
    # Convert obstacles to serializable format: list of [x, y, z, radius] for 3D
    obstacles_config = []
    for pos, radius in obstacles:
        if len(pos) == 3:
            obstacles_config.append([pos[0], pos[1], pos[2], radius])  # [x, y, z, radius]
        else:
            # Pad or truncate to 3D
            pos_3d = [pos[0] if len(pos) > 0 else 0.0,
                     pos[1] if len(pos) > 1 else 0.0,
                     pos[2] if len(pos) > 2 else 0.0]
            obstacles_config.append(pos_3d + [radius])
    ...
    for goal in goals:
        ...
        goals_config.append(goal_list[:3])
    ...
    initial_position_list = initial_position_list[:3]
