#!/usr/bin/env python3
"""
CMG-specific evaluation suite runner.
Includes topology necessity tests.
"""
...
def compute_mode_centroids(actor, env, config, n_samples_per_mode=20):
    """
    Compute mode centroids from encoder (for CMG diagnostics).
    """
    ...
    for target_mode in range(config.K):
        ...
        while len(samples) < n_samples_per_mode and attempts < 200:
            from v2.environments.cmg.episode import generate_episode
            ...
            if k == target_mode:
                s0 = torch.tensor(episode['obs'][0], dtype=torch.float32, device=device)
                traj = torch.tensor(episode['x'][1:], dtype=torch.float32, device=device)
        ...
        if len(samples) > 0:
            ...
            mode_centroids[target_mode] = torch.tensor(centroid, dtype=torch.float32, device=device)
    
    return mode_centroids


def main():
    parser = argparse.ArgumentParser(description="Run CMG evaluation suite with topology tests")
    ...
    # Run CMG topology diagnostics
    print("\n" + "="*60)
    print("CMG TOPOLOGY DIAGNOSTICS")
    print("="*60)
    
    # Compute mode centroids
    mode_centroids = compute_mode_centroids(runner.actor, runner.env, runner.env.config)
    
    # Convert to numpy for diagnostics
    mode_centroids_np = {k: v.cpu().numpy() for k, v in mode_centroids.items()}
    
    # Run topology suite
    topology_results = run_cmg_topology_suite(
        runner.actor,
        runner.env,
        runner.env.config,
        mode_centroids_np,
    ...
    print(f"\nUniversal Suite: {len(suite_results)} components")
    print(f"CMG Topology Tests: {len(topology_results)} tests")
